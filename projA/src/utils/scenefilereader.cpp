...
bool ScenefileReader::parseCameraData(const QJsonObject &cameradata) {
    ...
    // Ensure look/up form an orthonormal basis so the view matrix is stable
    glm::vec3 lookVec = glm::normalize(glm::vec3(m_cameraData.look));
    glm::vec3 upVec   = glm::normalize(glm::vec3(m_cameraData.up));
    glm::vec3 right   = glm::cross(upVec, lookVec);
    if (glm::length(right) < 1e-5f) {
        right = glm::vec3(1.f, 0.f, 0.f);
    } else {
        right = glm::normalize(right);
    }
    upVec = glm::normalize(glm::cross(lookVec, right));
    m_cameraData.look = glm::vec4(lookVec, 0.f);
    m_cameraData.up   = glm::vec4(upVec, 0.f);
